...
if (isset($_GET['view'])) {
    ...
    if (LOGIN_UserType == "Admin" || LOGIN_UserType == "Digital") {
        $LeadSql = "SELECT * FROM data where DataPersonFullname like '%$DataPersonFullName%'  and DataPersonPhoneNumber like '%$DataPersonPhoneNumber%' and DataPersonStatus like '%$DataPersonStatus%' and DataPersonSource like '%$DataPersonSource%' and DataType like '%LEAD%' $Managed $date GROUP BY DataId ORDER BY DataId DESC";
    } else {
        $LOGIN_UserId = LOGIN_UserId;
        $LeadSql = "SELECT * FROM data where DataPersonManagedBy='$LOGIN_UserId' and DataPersonFullname like '%$DataPersonFullName%'  and DataPersonPhoneNumber like '%$DataPersonPhoneNumber%' and DataPersonStatus like '%$DataPersonStatus%' and DataPersonSource like '%$DataPersonSource%' and DataType like '%LEAD%'  $date GROUP BY DataId ORDER BY DataId DESC";
    }
} elseif (isset($_GET['sub_status'])) {
    $sub_status = $_GET['sub_status'];
    if (LOGIN_UserType == "Admin" || LOGIN_UserType == "Digital") {
        $LeadSql = "SELECT DataPersonCreatedBy, DataPersonSubStatus, DataPersonEmailId,  DataPersonPhoneNumber, DataPersonStatus, DataSalutations, DataPersonFullname, DataPersonManagedBy, DataPersonSource, DataPriorityLevel, DataPersonCreatedAt, DataId FROM 'data' WHERE DataPersonSubStatus like '%$sub_status%' and DataType like '%LEAD%'  GROUP BY DataId ORDER by DataId DESC ";
    } else {
        $LOGIN_UserId = LOGIN_UserId;
        $LeadSql = "SELECT DataPersonCreatedBy, DataPersonSubStatus, DataPersonEmailId,  DataPersonPhoneNumber, DataPersonStatus, DataSalutations, DataPersonFullname, DataPersonManagedBy, DataPersonSource, DataPriorityLevel, DataPersonCreatedAt, DataId  FROM 'data' where DataPersonSubStatus like '%$sub_status%' and DataType like '%LEAD%' and DataPersonManagedBy='$LOGIN_UserId'   GROUP BY DataId ORDER by DataId DESC ";
    }
} else {
    if (LOGIN_UserType == "Admin" || LOGIN_UserType == "Digital") {
        $LeadSql = "SELECT DataPersonCreatedBy, DataPersonSubStatus, DataPersonEmailId,  DataPersonPhoneNumber, DataPersonStatus, DataSalutations, DataPersonFullname, DataPersonManagedBy, DataPersonSource, DataPriorityLevel, DataPersonCreatedAt, DataId FROM data WHERE DataType like '%LEAD%'  GROUP BY DataId ORDER by DataId DESC ";
    } else {
        $LOGIN_UserId = LOGIN_UserId;
        $LeadSql = "SELECT DataPersonCreatedBy, DataPersonSubStatus, DataPersonEmailId,  DataPersonPhoneNumber, DataPersonStatus, DataSalutations, DataPersonFullname, DataPersonManagedBy, DataPersonSource, DataPriorityLevel, DataPersonCreatedAt, DataId  FROM data where DataPersonManagedBy='$LOGIN_UserId' and DataType like '%LEAD%' GROUP BY DataId ORDER by DataId DESC ";
    }
}
